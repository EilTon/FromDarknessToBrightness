...
public class Player2Controller : MonoBehaviour
{
	...
	void MoveShieldTrigger()
	{
		...
		if (_player1.GetComponent<PlayerController>().GetDetach()==false)
		{
			...
			//_colliderPlayer.transform.position = _player1.transform.position + (Vector3)p1Top2 * _shieldDistance;
			_animationManager.SetCourbe(_currentShieldAngle / _minMaxShieldAngle.y);
			//_player1.transform.position = _player1.transform.position + (Vector3)p1Top2 * _shieldDistance;
