...
namespace Railcam2D
{
	...
	public class Railcam2DCore : MonoBehaviour
	{
				...
				if (_triggers[i].Active && _triggers[i].TargetDetected)
				{
					...
					switch (_currentTrigger.Event)
					{
						...
						case (TriggerEvent.ChangeSizeCamera):
							...
							//ChangeSizeCameraNoCouroutine(_currentTrigger.TimeToSize, _currentTrigger.SizeCamera);
							break;
						default:
							return _currentRail;
					}
		...
		//		_camera.orthographicSize = Mathf.SmoothStep(oldSize, newSize, elapsed);
		//	}
		//}
