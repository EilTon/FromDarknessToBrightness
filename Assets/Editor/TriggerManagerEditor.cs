...
namespace Railcam2D
{
	...
	public class TriggerManagerEditor : Editor
	{
		...
		private void DrawMainEditor(Trigger trigger, string heading)
		{
			...
			//New size Camera
			if (trigger != null && (trigger.Event == TriggerEvent.ChangeSizeCamera))
			{
				_content = new GUIContent("New size Camera", "Change the size of camera");
				var newSizeCamera = EditorGUILayout.FloatField(_content, currentSizeCamera);
